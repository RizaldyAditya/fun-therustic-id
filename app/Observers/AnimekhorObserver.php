...
class AnimekhorObserver extends CrawlObserver
{
    use Utilities;
        ...
        $domCrawler->filter('.listupd.normal > .excstf > article')->each(function (DomCrawler $node) {
            try {
                ...
                if ($donghua_id) {
                    ...
                    // update donghua -> external_titles and animekhor_url
                    Donghua::where('id', $donghua_id)
                        ->where(function ($query) {
                            $query->whereNull('external_titles->animekhor_url')
                                ->orWhere('external_titles->animekhor_url', '');
                        })
                        ->update([
                            'external_titles->animekhor_url' => $allEpisodes ?? '',
                        ]);
