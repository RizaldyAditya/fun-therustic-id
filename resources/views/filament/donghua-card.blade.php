    ...
    <div class="relative aspect-[3/4] w-full overflow-hidden">
        <a href="{{ $getRecord()->stream_url }}" class="absolute inset-0 z-11" target="_blank"></a>
            ...
            <div class="absolute top-0 left-0 z-10">
                <div class="bg-yellow-500 text-black text-[14px] font-black px-3 py-1 rounded-br-lg shadow-lg flex items-center gap-1 uppercase italic">
                    <span>EP {{ $getRecord()->episode_number }}</span>
                </div>
            </div>
            
            <div class="absolute top-2 right-2 z-10">
                <span class="bg-black/60 backdrop-blur-md text-white text-[12px] px-2 py-0.5 rounded border border-white/10 font-bold">
                    Season {{ $getRecord()->donghua->season }}
                </span>
            </div>

            <div class="absolute bottom-2 right-2 z-10">
                <div class="bg-black/60 backdrop-blur-sm p-1 rounded-md border border-white/10 flex items-center justify-center shadow-2xl">
                    @if($getRecord()->stream->logo) {{-- Assuming you have a logo_path in your stream model --}}
                        <img src="{{ asset('storage/' . $getRecord()->stream->logo) }}" 
                            alt="{{ $getRecord()->stream->name }}"
                            class="h-6 w-auto object-contain" />
                    @else
                        <span class="text-[9px] text-white font-black uppercase px-1">
